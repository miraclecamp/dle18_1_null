...
HTML;
		...
		if ($is_logged) {
			...
			if ( $member_id['pm_unread'] > intval ( $_COOKIE['dle_newpm'] ) ) {
				...
				$onload_scripts[] = "DLEPush.info('{$lang['pm_alert']} <a href=\"{$PHP_SELF}?do=pm\">{$lang['pm_aread']}</a>', null, 60000);";
...
if( $config['allow_social'] AND $config['allow_registration'] AND (!$is_logged OR ($is_logged AND $dle_module == 'userinfo' AND $user == $member_id['name'] ) ) ) {
	...
	if ( $social_config['vk'] ) {
		...
		$vk_url = 'https://id.vk.com/authorize'.'?' . http_build_query($social_params, '', '&amp;');
