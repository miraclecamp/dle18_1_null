...
if (isset($_REQUEST['searchword']) AND $_REQUEST['searchword']) {
  ...
  $searchword = htmlspecialchars( strip_tags( stripslashes( trim( urldecode ( $_REQUEST['searchword'] ) ) ) ), ENT_QUOTES, 'UTF-8' );
...
if( $_POST['action'] == "doadd" ) {
	...
	$comments_place = isset($_REQUEST['comments_place']) ? intval($_REQUEST['comments_place']) : 0;
	...
	$db->query( "INSERT INTO " . PREFIX . "_banners (banner_tag, descr, code, approve, short_place, bstick, main, category, grouplevel, start, end, fpage, innews, devicelevel, allow_views, max_views, allow_counts, max_counts, rubric, comments_place, allowed_country, not_allowed_country) values ('{$banner_tag}', '{$banner_descr}', '{$banner_code}', '{$approve}', '{$short_place}', '{$bstick}', '{$main}', '{$category}', '{$grouplevel}', '{$start_date}', '{$end_date}', '{$fpage}', '{$innews}', '{$devicelevel}', '{$allow_views}', '{$max_views}', '{$allow_counts}', '{$max_counts}', '{$rub_id}', '{$comments_place}', '{$allowed_country}', '{$not_allowed_country}')" );
...
if( $_POST['action'] == "addrubric" ) {
	...
	$title  = $db->safesql(  htmlspecialchars( strip_tags( stripslashes($_POST['title'] ) ), ENT_QUOTES, 'UTF-8') );
	$description  = $db->safesql(  htmlspecialchars( strip_tags( stripslashes($_POST['description'] ) ), ENT_QUOTES, 'UTF-8') );
...
if( $_REQUEST['action'] == "add" or $_REQUEST['action'] == "edit" ) {
	...
	if( $_REQUEST['action'] == "add" ) {
		...
		$banner_descr = htmlspecialchars( $row['descr'], ENT_QUOTES, 'UTF-8' );
		$banner_code = htmlspecialchars(  $row['code'], ENT_QUOTES, 'UTF-8' );
		$short_place = $row['short_place'];
		$comments_place = $row['comments_place'];
		...
		$allowed_country = ($row['allowed_country']) ? htmlspecialchars($row['allowed_country'], ENT_QUOTES, 'UTF-8') : "";
		$not_allowed_country = ($row['not_allowed_country']) ? htmlspecialchars($row['not_allowed_country'], ENT_QUOTES, 'UTF-8') : "";
	...
	echo <<<HTML
<script src="engine/editor/jscripts/ace/code_editor.js"></script>
<script src="engine/editor/jscripts/ace/langs/{$lang['language_code']}.js"></script>
...
<div class="panel panel-default">
  ...
  <div class="panel-body">
		...
		<div class="form-group editor-group">
		  ...
		  <div class="col-md-10 col-sm-9">
			<div style="border: solid 1px #BBB;width:100%;">
				<textarea dir="auto" style="width:100%;" class="classic" name="banner_code" id="banner_code" rows="16">{$banner_code}</textarea>
			</div>
		  </div>
		 </div>
		<div class="form-group">
		  <label class="control-label col-md-3 col-sm-3">{$lang['stat_allow']}</label>
		...
		<div class="form-group">
		  <label class="control-label col-md-3 col-sm-3">{$lang['banner_dev']}</label>
		  <div class="col-md-9 col-sm-9">
		...
		<div class="form-group">
		  <label class="control-label col-md-3 col-sm-3">{$lang['banner_country']}</label>
		  <div class="col-md-9 col-sm-9">
		...
		<div class="form-group">
		  <label class="control-label col-md-3 col-sm-3">{$lang['banner_country_1']}</label>
		  <div class="col-md-9 col-sm-9">
		...
		<div class="form-group">
		  <label class="control-label col-md-3 col-sm-3">{$lang['banner_views']}</label>
		  <div class="col-md-9 col-sm-9">
			{$allow_views}
		  </div>
		</div>
		<div class="form-group">
		  <label class="control-label col-md-3 col-sm-3">{$lang['banner_counts']}</label>
		  <div class="col-md-9 col-sm-9">
			{$allow_counts}
		  </div>
		</div>
		<div class="form-group">
		  <label class="control-label col-md-3 col-sm-3">{$lang['banner_mviews']}</label>
		  <div class="col-md-9 col-sm-9">
			<input type="text" dir="auto" name="max_views" class="form-control" style="max-width:220px;" value="{$max_views}"><i class="help-button visible-lg-inline-block text-primary-600 fa fa-question-circle position-right position-left" data-rel="popover" data-trigger="hover" data-placement="auto right" data-content="{$lang['hint_bviews']}" ></i>
		  </div>
		</div>
		<div class="form-group">
		  <label class="control-label col-md-3 col-sm-3">{$lang['banner_mcounts']}</label>
		  <div class="col-md-9 col-sm-9">
		...
		<div class="form-group">
		  <label class="control-label col-md-3 col-sm-3">{$lang['banners_s_opt']}</label>
		  <div class="col-md-9 col-sm-9">
HTML;
...
echo <<<HTML
			<i class="help-button visible-lg-inline-block text-primary-600 fa fa-question-circle position-right position-left" data-rel="popover" data-trigger="hover" data-placement="auto right" data-content="{$lang['banners_s']}" ></i>
		  </div>
		</div>

		<div class="form-group">
		  <label class="control-label col-md-3 col-sm-3">{$lang['banners_c_opt']}</label>
		  <div class="col-md-9 col-sm-9">
HTML;
	echo makeDropDown(array("0" => $lang['banners_s_0'], "1" => $lang['banners_s_1'], "2" => $lang['banners_s_2'], "3" => $lang['banners_s_3'], "4" => $lang['banners_s_4'], "5" => $lang['banners_s_5'], "6" => $lang['banners_s_6'], "7" => $lang['banners_s_7']), "comments_place", $comments_place);

	echo <<<HTML
			<i class="help-button visible-lg-inline-block text-primary-600 fa fa-question-circle position-right position-left" data-rel="popover" data-trigger="hover" data-placement="auto right" data-content="{$lang['banners_c']}" ></i>
		  </div>
		</div>
		...
		<div class="form-group">
		  <div class="col-md-12 col-sm-12">
			...
			<div class="checkbox"><label><input class="icheck" type="checkbox" value="1" name="innews" {$checked6} id="innews" />{$lang['banners_innews']}</label></div>
			 <div class="checkbox"><label><input class="icheck" type="checkbox" value="1" name="bstick" {$checked3} id="bstick" />{$lang['banners_bstick']}</label></div>  
		</div>
		</div>
...
HTML;
	...
	while ( $row = $db->get_row() ) {
		...
		$row['descr'] = $row['descr'];
		$row['code'] = "<pre><code>".htmlspecialchars ($row['code'], ENT_QUOTES, 'UTF-8')."</code></pre>";
...
function confirmdelete(id){
	    DLEconfirmDelete( '{$lang['banners_del']}', '{$lang['p_confirm']}', function () {
...
function confirm_rubric_delete(id){
	    DLEconfirmDelete( '{$lang['rubric_del']}', '{$lang['p_confirm']}', function () {
