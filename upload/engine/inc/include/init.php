...
define('DINITVERSION', '0465' );
define('VERSIONID',    '18.1' );
define('BUILDID',      '106' );
...
if (isset($config['allowed_panel_country']) AND trim($config['allowed_panel_country']) AND $mod != "upgrade") {
	if (!DLECountry::Check($config['allowed_panel_country'])) $block_country = true;
}

if (isset($config['declined_panel_country']) AND trim($config['declined_panel_country']) AND $mod != "upgrade") {
