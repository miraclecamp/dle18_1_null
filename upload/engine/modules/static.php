...
HTML;
		...
		if( $static_result['allow_template'] OR $view_template == "print" ) {
			...
			$compare_date = compare_days_date( $static_result['date'] );
		...
		if ($config['allow_links'] AND $view_template != "print" AND function_exists('replace_links') AND isset($replace_links['static'])){
			$tpl->result['content'] = replace_links($tpl->result['content'], $replace_links['static']);
		}
		...
		foreach($data as $url) {
			...
			if (isset($info['extension'])) {
				if ($info['filename'] == "spoiler-plus" OR $info['filename'] == "spoiler-minus" OR strpos($info['dirname'], 'engine/data/emoticons') !== false) continue;
				$info['extension'] = strtolower($info['extension']);
				if ( in_array($info['extension'], $img_arr) AND !in_array($url, $images) ) array_push($images, $url);
			}
		}
	...
	if( $config['own_404'] AND file_exists(ROOT_DIR . '/404.html') ) {
		...
		$lang['static_page_err'] = str_replace ("{page}", htmlspecialchars(stripslashes($name), ENT_QUOTES, 'UTF-8' ).".html", $lang['static_page_err']);
