...
-----------------------------------------------------
 https://dle-news.ru/
-----------------------------------------------------
 Copyright (c) 2009-2025 IT-Security (Asafov Sergey)
=====================================================
...
if(!class_exists('DLE_API'))
{
	class DLE_API
	{
		...
		function change_user_name ($user_id, $new_name)
		{
			...
			if( !is_array($count_arr) ) return false;
		...
		function external_register($login, $password, $email, $group)
		{
			...
			$now = time();
			$q = $this->db->query( "insert into " . USERPREFIX . "_users (email, password, name, user_group, reg_date, lastdate) VALUES ('$email', '$password', '$login', '$group', '$now', '$now')" );
			return 1;
		}		
		...
		function send_pm_to_user($user_id, $subject, $text, $from)
		{
			...
			$row = $this->db->super_query("SELECT user_id FROM " . USERPREFIX . "_users WHERE name='{$from}'");
			$row['user_id'] = isset($row['user_id']) ? $row['user_id'] : 0;
			
			if( !$row['user_id'] ) {
				return 0;
			}

			$this->db->query("INSERT INTO " . USERPREFIX . "_conversations (subject, created_at, updated_at, sender_id, recipient_id) values ('{$subject}', '{$now}', '{$now}', '{$row['user_id']}', '{$user_id}')");
			$conversation_id = $this->db->insert_id();
			$this->db->query("INSERT INTO " . USERPREFIX . "_conversation_users (user_id, conversation_id) values ('{$user_id}', '{$conversation_id}') ON DUPLICATE KEY UPDATE user_id = VALUES(user_id)");
			$this->db->query("INSERT INTO " . USERPREFIX . "_conversations_messages (conversation_id, sender_id, content, created_at) values ('{$conversation_id}', '{$row['user_id']}', '{$text}', '{$now}')");
		...
		function load_table ($table, $fields = "*", $where = '1', $multirow = false, $start = 0, $limit = 0, $sort = '', $sort_order = 'desc')
		{
			...
			if (is_array($values) AND count($values)>0) return $values;
		...
		function save_to_cache ($fname, $vars)
		{
			...
			@fwrite($f, $vars);
			@fclose($f);
			@chmod($this->cache_dir.$filename, 0666);
			return $vars;
		}
