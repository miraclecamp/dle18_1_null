...
if( !isset( $recipient['user_id'] ) ) {
	echo "{\"status\": \"error\",\"text\": \"{$lang['feed_err_8']}\"}";
	die();
}

if ( $recipient['banned'] ) {
	echo "{\"status\": \"error\",\"text\": \"{$lang['feedback_banned']}\"}";
	die();
}
...
if( empty($email) OR dle_strlen($email) > 50 OR @count(explode("@", $email)) != 2) {
...
if( $stop ) {
	...
	if( $mail->send_error ) {
		...
		msgbox( $lang['feed_ok_1'], "{$lang['feed_ok_2']} <a href=\"{$_ROOT_DLE_URL}\">{$lang['feed_ok_4']}</a>" );

		$tpl->result['info'] = str_replace( '{THEME}', $_ROOT_DLE_URL . 'templates/' . $config['skin'], $tpl->result['info'] );
		
		echo json_encode(array("status" => 'ok', "text" => $tpl->result['info']), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
